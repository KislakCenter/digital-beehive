...
{% for item in site.data.toc_vol2 %}
  ...
  <div id='divthumbs'>
    <div class= 'divthumb'>
      <hr>
        <a href="{{ site.baseurl }}/toc_vol2/{{ item.pid }}"><img style="padding: 0 15px; float: left; clear: both; margin-top: 10px" src="{{ item.thumbnail }}"/></a>
          <div class="label">
          {% if item.toc_title %}
          ...
          {% endif %}
          {% if item.first_entry %}
            <strong>Content:</strong> {{ item.first_entry }} -
          {% endif %}
          {% if item.last_entry %}
            {{ item.last_entry }} <br>
          {% endif %}
